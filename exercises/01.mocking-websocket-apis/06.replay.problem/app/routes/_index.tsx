...
import { ChatMessage, User } from '~/components/chat-message'
...
export default function Homepage() {
  ...
  const { isConnectionOpen, getClient } = useWebSocket(
    'wss://example.com/chat',
    {
      onMessage(event) {
        ...
        /**
         * @fixme @todo Make this communication type-safe.
         */
        switch (message.type) {
          case 'message': {
  ...
  const submitMessageForm = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (mediaFile != null && mediaFile instanceof File && mediaFile.size > 0) {
  ...
  return (
    <div className="flex flex-col h-full">
      <JoinUserDialog open={!user} onJoin={handleUserJoin} />
      ...
      <ScrollArea viewportRef={messagesScrollAreaRef} className="flex-grow p-6">
      ...
      <form onSubmit={submitMessageForm} className="p-6 border-t bg-muted/30">
        <fieldset className="flex flex-col gap-4" disabled={!isConnectionOpen}>
          ...
          <footer className="flex items-center self-end gap-6">
            {uploadedMedia ? (
              <Badge variant="secondary">{uploadedMedia.name}</Badge>
            ) : null}
